...
void timer_change_notify(long next) { globalTimer->startTimerForTick(next); }
...
int GuiTerminalWindow::TranslateKey(QKeyEvent *keyevent, char *output) {
  ...
  if (cfg->funky_type == FUNKY_VT400 && code <= 6) code = "\0\2\1\4\5\3\6"[code];
...
int filename_equal(Filename f1, Filename f2) { return !strcmp(f1.path, f2.path); }

int filename_is_null(Filename fn) { return !*fn.path; }
...
void qutty_connection_fatal(void *frontend, char *msg) {
  ...
  if (f->cfg.close_on_exit == FORCE_ON) f->closeTerminal();
...
void notify_remote_exit(void *frontend) {
  ...
  if (exitcode >= 0) {
    if (f->cfg.close_on_exit == FORCE_ON || (f->cfg.close_on_exit == AUTO && exitcode != INT_MAX)) {
      f->closeTerminal();
    } else {
