...
qutty_menu_actions_t qutty_menu_actions[MENU_STATIC_ACTION_MAX] = {
    ...
    {"Close", "", SLOT(contextMenuCloseSessionTriggered()), "Close currently active session/pane"},
    ...
    {"Switch to Left Tab", "Shift+Left", SLOT(tabPrev()), "Switch to the tab in the left"},
    {"Switch to Right Tab", "Shift+Right", SLOT(tabNext()), "Switch to the tab in the right"},
    ...
    {"Export To File", "", SLOT(contextMenuExportToFile()), "Export the QuTTY sessions to file"},
    {"Exit", "", SLOT(contextMenuCloseWindowTriggered()), "Close the window"},
    {"Show Menubar", "", SLOT(contextMenuMenuBar()), "Toggle viewing of menu bar"},
    {"Fullscreen", "", SLOT(contextMenuFullScreen()), "Toggle fullscreen mode"},
    {"Always on top", "", SLOT(contextMenuAlwaysOnTop()), "Toggle keeping QuTTY window on top"},
...
qutty_menu_links_t qutty_menu_links[MENU_MAX_MENU] = {
    {"File",
     10,
     {MENU_NEW_TAB, MENU_NEW_WINDOW, MENU_SEPARATOR, MENU_SAVED_SESSIONS, MENU_SEPARATOR,
      MENU_SPLIT_SESSION, MENU_SEPARATOR, MENU_EXPORT_IMPORT, MENU_SEPARATOR, MENU_EXIT}},
    {"Edit",
     9,
     {MENU_PASTE, MENU_SEPARATOR, MENU_RENAME_TAB, MENU_SEPARATOR, MENU_FIND, MENU_FIND_NEXT,
      MENU_FIND_PREVIOUS, MENU_SEPARATOR, MENU_PREFERENCES}},
    {"View",
     10,
     {MENU_SWITCH_LEFT_TAB, MENU_SWITCH_RIGHT_TAB, MENU_SEPARATOR, MENU_SWITCH_UP_PANE,
      MENU_SWITCH_BOTTOM_PANE, MENU_SEPARATOR, MENU_MENUBAR, MENU_TAB_IN_TITLE_BAR,
      MENU_ALWAYSONTOP, MENU_FULLSCREEN}},
    ...
    {"Menu Term Window",
     10,
     {MENU_PASTE, MENU_SEPARATOR, MENU_NEW_TAB, MENU_RESTART_SESSION, MENU_DUPLICATE_SESSION,
      MENU_SAVED_SESSIONS, MENU_SPLIT_SESSION, MENU_CHANGE_SETTINGS, MENU_SEPARATOR,
      MENU_RENAME_TAB, MENU_SEPARATOR, MENU_CLOSE_SESSION}},
    {"Menu Tabbar",
     14,
     {MENU_NEW_TAB, MENU_NEW_WINDOW, MENU_SEPARATOR, MENU_SAVED_SESSIONS, MENU_SEPARATOR,
      MENU_SPLIT_SESSION, MENU_SEPARATOR, MENU_EXPORT_IMPORT, MENU_SEPARATOR, MENU_VIEW,
      MENU_SEPARATOR, MENU_EDIT, MENU_SEPARATOR, MENU_EXIT}},
...
class MyStyle : public QProxyStyle {
 ...
 public:
  MyStyle(GuiMainWindow *wnd) { mainWindow = wnd; }

  QRect subElementRect(SubElement element, const QStyleOption *option,
                       const QWidget *widget = 0) const {
    QRect rc = QProxyStyle::subElementRect(element, option, widget);
    if (element == QStyle::SE_TabWidgetLeftCorner || element == QStyle::SE_TabWidgetRightCorner) {
...
void GuiMainWindow::initializeMenuSystem() {
  for (int i = 0; i < MENU_STATIC_ACTION_MAX; i++) {
    if (!qutty_menu_actions[i].name) continue;
    QKeySequence keyseq(qutty_menu_actions[i].key);
    auto it = qutty_config.menu_action_list.find(i);
    if (it != qutty_config.menu_action_list.end()) keyseq = it->second.shortcut;
  ...
  connect(&qutty_config, SIGNAL(savedSessionsChanged()), this,
          SLOT(contextMenuSavedSessionsChanged()));
...
QAction *GuiMainWindow::menuGetActionById(qutty_menu_id_t id) {
  ...
  if (it != menuCommonShortcuts.end() && std::get<0>(*it) == id) return std::get<2>(*it);
  return NULL;
}
...
QKeySequence GuiMainWindow::menuGetShortcutById(qutty_menu_id_t id) {
  auto it = std::lower_bound(menuCommonShortcuts.begin(), menuCommonShortcuts.end(),
                             std::make_tuple(id, (QShortcut *)NULL, (QAction *)NULL));
  if (it != menuCommonShortcuts.end() && std::get<0>(*it) == id && std::get<1>(*it))
    return std::get<1>(*it)->key();
  return QKeySequence();
}

void GuiMainWindow::initializeCustomSavedSessionShortcuts() {
  for (auto it = menuCommonShortcuts.begin(); it != menuCommonShortcuts.end();) {
...
void GuiMainWindow::contextMenuCloseWindowTriggered() { this->close(); }
...
void GuiMainWindow::contextMenuTabInTitleBar() {
  ...
  menuGetActionById(MENU_TAB_IN_TITLE_BAR)
      ->setChecked(qutty_config.mainwindow.titlebar_tabs = true);
}
...
void GuiMainWindow::contextMenuTermTopDragPaneTriggered() {
  if (!menuCookieTermWnd) return;
  if (terminalList.indexOf(menuCookieTermWnd) == -1) return;
...
void GuiToolbarTerminalTop::initializeToolbarTerminalTop(GuiMainWindow *p) {
  ...
  connect(&btns[MENU_TERMTOP_MOVE], SIGNAL(pressed()), p,
          SLOT(contextMenuTermTopDragPaneTriggered()));
  connect(&btns[MENU_TERMTOP_CLOSE], SIGNAL(clicked()), p,
          SLOT(contextMenuTermTopCloseTriggered()));
...
void GuiMainWindow::contextMenuRenameTab() {
  ...
  if (ok && !text.isEmpty()) term->setCustomSessionTitle(text);
}
...
void GuiMainWindow::contextMenuAutoComplete() {
  ...
  if (!textFilterWnd) textFilterWnd = new GuiTextFilterWindow(this, true, "PREDEFINED_LIST");
...
void GuiMainWindow::contextMenuPasteHistory() {
  if (!this->getCurrentTerminal()) return;
  if (!textFilterWnd) textFilterWnd = new GuiTextFilterWindow(this, true, "PASTE_HISTORY");
  textFilterWnd->move((this->width() - textFilterWnd->width()) / 2,
                      (this->height() - textFilterWnd->height()) / 2);
  textFilterWnd->show();
  textFilterWnd->init();
