...
static void ssh_special(Backend *be, SessionSpecialCode code, int argl);
...
struct Ssh {
     ...
     */
    struct SessionSpecial *specials;
...
static void ssh1_log_outgoing_packet(Ssh *ssh, struct Packet *pkt)
{
    ...
    if ((pkt->type == SSH1_CMSG_AUTH_PASSWORD ||
        ...
        conf_get_bool(ssh->conf, CONF_logomitpass)) {
			 ...
			 struct Packet *pktin)
{
	...
	if (conf_get_bool(ssh->conf, CONF_tryagent) && agent_exists() && !s->tried_agent) {
	    /*
...
{
    return conf_get_bool(ssh->conf, CONF_agentfwd) && agent_exists();
}
			       ...
			       struct Packet *pktin)
{
    ...
    if (conf_get_bool(ssh->conf, CONF_x11_forward)) {
        ssh->x11disp =
    ...
    if (!conf_get_bool(ssh->conf, CONF_nopty)) {
	struct Packet *pkt;
    ...
    if (conf_get_bool(ssh->conf, CONF_compression)) {
    ...
    if (conf_get_bool(ssh->conf, CONF_ssh_subsys) &&
    ...
    if (ssh->eof_needed)
    ssh_special(&ssh->backend, SS_EOF, 0);
			     ...
			     struct Packet *pktin)
{
  ...
  begin_key_exchange:
	...
	for (i = 0; i < CIPHER_MAX; i++) {
		...
		break;
	      case CIPHER_DES:
        if (conf_get_bool(ssh->conf, CONF_ssh2_des_cbc)) {
...
static int ssh_is_simple(Ssh *ssh)
{
     ...
     */
    return (conf_get_bool(ssh->conf, CONF_ssh_simple) &&
...
{
    if (ssh->version == 2 &&
        !conf_get_bool(ssh->conf, CONF_ssh_no_shell) &&
...
static void ssh2_msg_userauth_banner(Ssh *ssh, struct Packet *pktin)
{
    ...
    if (conf_get_bool(ssh->conf, CONF_ssh_show_banner) &&
			     ...
			     struct Packet *pktin)
{
	...
	if (conf_get_bool(ssh->conf, CONF_tryagent) && agent_exists()) {
	 ...
	 * Get a username.
	 */
	if (s->got_username && !conf_get_bool(ssh->conf, CONF_change_username)) {
	    /*
	...
	while (1) {
		...
		if (!ssh2_pkt_getbool(pktin)) {
			...
			if (conf_get_bool(ssh->conf, CONF_change_username)) {
		...
		s->can_keyb_inter = conf_get_bool(ssh->conf, CONF_try_ki_auth) &&
		    in_commasep_string("keyboard-interactive", methods, methlen);
#ifndef NO_GSSAPI
                if (conf_get_bool(ssh->conf, CONF_try_gssapi_auth) &&
     ...
     */
    if (conf_get_bool(ssh->conf, CONF_ssh_no_shell)) {
	ssh->mainchan = NULL;
    } else {
	...
	while (1) {
	    ...
	    if (ssh->fallback_cmd) {
        subsys = conf_get_bool(ssh->conf, CONF_ssh_subsys2);
		cmd = conf_get_str(ssh->conf, CONF_remote_cmd2);
	    } else {
        subsys = conf_get_bool(ssh->conf, CONF_ssh_subsys);
...
{
    ssh->logomitdata = conf_get_bool(ssh->conf, CONF_logomitdata);
}
 ...
 * protocol.
 */
static const struct SessionSpecial *ssh_get_specials(Backend *be)
{
    static const struct SessionSpecial ssh1_ignore_special[] = {
    {"IGNORE message", SS_NOP}
    };
    static const struct SessionSpecial ssh2_ignore_special[] = {
    {"IGNORE message", SS_NOP},
    };
    static const struct SessionSpecial ssh2_rekey_special[] = {
    {"Repeat key exchange", SS_REKEY},
    };
    static const struct SessionSpecial ssh2_session_specials[] = {
    {NULL, SS_SEP},
    {"Break", SS_BRK},
    ...
    static const struct SessionSpecial specials_end[] = {
    {NULL, SS_EXITMENU}
    };

    struct SessionSpecial *specials = NULL;
...
#define ADD_SPECIALS(name) do                                           \
    {                                                                   \
        ...
        if (nspecials + len > specialsize) {                            \
            ...
            specials = sresize(specials, specialsize, struct SessionSpecial); \
        }                                                               \
    memcpy(specials+nspecials, name, len*sizeof(struct SessionSpecial)); \
        nspecials += len;                                               \
    } while (0)
	...
	if (ssh->mainchan)
        ...
        if (ssh->n_uncert_hostkeys) {
            static const struct SessionSpecial uncert_start[] = {
                {NULL, SS_SEP},
                {"Cache new host key type", SS_SUBMENU},
            };
            static const struct SessionSpecial uncert_end[] = {
                {NULL, SS_EXITMENU},
            };
            int i;

            #if 0 // TODO will be taken care of in upcoming merges with 0.71
            ADD_SPECIALS(uncert_start);
            for (i = 0; i < ssh->n_uncert_hostkeys; i++) {
                struct SessionSpecial uncert[1];
                const struct ssh_signkey *alg =
                    hostkey_algs[ssh->uncert_hostkeys[i]].alg;
                uncert[0].name = alg->name;
                uncert[0].code = SS_LOCALSTART + ssh->uncert_hostkeys[i];
                ADD_SPECIALS(uncert);
            }
...
#if 0 // will be resolved in future merges with 0.71
