...
int proxy_socks5_handlechap(ProxySocket p) {
  ...
  while (p->chap_num_attributes == 0 || p->chap_num_attributes_processed < p->chap_num_attributes) {
    ...
    if (p->chap_num_attributes == 0) {
      ...
      switch (p->chap_current_attribute) {
        ...
        case 0x03:
          ...
          hmacmd5_chap(data, p->chap_current_datalen, conf_get_str(p->conf, CONF_proxy_password),
                       &outbuf[4]);
    ...
    if (p->state == 8 && p->chap_num_attributes_processed >= p->chap_num_attributes) {
