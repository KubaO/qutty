...
GuiBase::SplitType GuiDragDropSite::updateDropMode(const QPoint &pos) {
  QWidget *w;
  if (!geometry().contains(pos)) goto cu0;
...
void GuiTerminalWindow::dragEnterEvent(QDragEnterEvent *e) {
  if (e->source() == this || !e->mimeData()->hasFormat("qutty-terminal-drag-drop-action")) {
    e->ignore();
    return;
  }
...
void GuiTabBar::dragLeaveEvent(QDragLeaveEvent *) { mainWindow->dragDropSite.clearDropMode(); }
...
void GuiTabBar::dropEvent(QDropEvent *e) {
  ...
  if (mainWindow->dragDropSite.get_drop_loc() != GuiDragDropSite::DRAG_DROP_ON_TABBAR ||
      (split != GuiBase::TYPE_LEFT && split != GuiBase::TYPE_RIGHT) || tabind == -1)
    goto cu0;
