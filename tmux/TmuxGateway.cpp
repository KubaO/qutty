...
TmuxGateway::~TmuxGateway() {
  if (_sessionName) sfree(_sessionName);
  closeAllPanes();
...
size_t TmuxGateway::fromBackend(int is_stderr, const char *data, size_t len) {
  ...
  if (bufchain_size(&buffer) > 0) {
    ...
    sfree(buf);
    if (rc == -1) goto tmux_disconnected;
  } else {
...
#define strStartsWith(start, s, l) (strlen(start) <= l && !strncmp(start, s, strlen(start)))
  ...
  if (strStartsWith("%end", command, len)) {
    ...
    if (_currentCommand.receiver) {
      _currentCommand.receiver->performCallback(_currentCommand.callback, _currentCommandResponse);
  ...
  } else if (strStartsWith("%exit ", command, len) || strStartsWith("%exit\n", command, len)) {
...
int TmuxGateway::cmd_hdlr_session_changed(const char *command, size_t len) {
  ...
  if (sessid == 0 && sessname == cmd) goto cu0;
...
int TmuxGateway::cmd_hdlr_window_close(const char *command, size_t len) {
  ...
  termGatewayWnd->getMainWindow()->tabArea->removeTab(
      termGatewayWnd->getMainWindow()->tabArea->indexOf(_mapPanes[paneid]->termWnd()));
...
int TmuxGateway::openWindowsInitial() {
  ...
  wsprintf(set_client_size, L"refresh-client -C %d,%d\n", termGatewayWnd->term->cols,
           termGatewayWnd->term->rows);
  sendCommand(this, CB_NULL, set_client_size);
  sendCommand(this, CB_NULL, L"list-sessions -F \"#{session_name}\"\n");
...
int TmuxGateway::createNewWindow(int id, const char *name, int /*width*/, int /*height*/,
                                 const string &layout) {
  if (!_mapLayout[id].initLayout(layout.substr(5))) goto cu0;
...
int TmuxGateway::createNewWindowPane(int id, const char *name, const TmuxLayout &layout,
    ...
    case TmuxLayout::TMUX_LAYOUT_TYPE_LEAF:
      if (_mapPanes.find(layout.paneid) == _mapPanes.end()) {
        ...
        wsprintf(
            cmd_state,
            L"list-panes -t %%%d -F \""
            L"pane_id=#{pane_id}\talternate_on=#{alternate_on}\talternate_saved_x=#{alternate_"
            L"saved_x}\t"
            ...
            L"scroll_region_upper=#{scroll_region_upper}\tscroll_region_lower=#{scroll_region_"
            L"lower}\t"
            L"pane_tabs=#{pane_tabs}\tcursor_flag=#{cursor_flag}\tinsert_flag=#{insert_flag}\t"
            L"keypad_cursor_flag=#{keypad_cursor_flag}\tkeypad_flag=#{keypad_flag}\twrap_flag=#{"
            L"wrap_flag}\t"
...
void TmuxGateway::detach() { closeAllPanes(); }
