...
bool TmuxLayout::parseLayout(istringstream &iresp) {
  ...
  if (iresp.peek() == '{' || iresp.peek() == '[') {
    ...
    layoutType = (openChar == '[') ? TmuxLayout::TMUX_LAYOUT_TYPE_VERTICAL
                                   : TmuxLayout::TMUX_LAYOUT_TYPE_HORIZONTAL;
    if (!parseLayoutChildren(iresp)) goto cu0;
  } else {
    if (iresp.get() != ',') goto cu0;
    iresp >> paneid;
...
int main() {
  const char *test[] = {
      ...
      "204x50,0,0[204x25,0,0,204x12,0,26,204x11,0,39{102x11,0,39,101x11,103,39[101x5,103,39,101x5,"
      "103,45{50x5,103,45,50x5,154,45}]}]",
