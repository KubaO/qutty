...
void GuiTerminalWindow::createSplitLayout(GuiBase::SplitType split, GuiTerminalWindow *newTerm) {
  ...
  if (!parentSplit) {
    ...
    int initsize = orient == Qt::Vertical ? viewport()->width() : viewport()->height();
...
void GuiSplitter::createSplitLayout(Qt::Orientation orient, GuiBase::SplitType split,
  ...
  int initsize =
      orient == Qt::Vertical ? oldTerm->viewport()->width() : oldTerm->viewport()->height();
...
void GuiSplitter::removeSplitLayout(GuiTerminalWindow *term) {
  ...
  if (parentSplit) {
    parentSplit->child.erase(
        std::remove(parentSplit->child.begin(), parentSplit->child.end(), this));
    ...
    if (currtab == tabind) term->getMainWindow()->tabArea->setCurrentIndex(tabind);
  }
  this->close();
  this->deleteLater();
}
...
GuiTerminalWindow *GuiSplitter::navigatePane(Qt::Key key, GuiTerminalWindow *tofind, int splitind) {
  if (orientation() == Qt::Horizontal && (key == Qt::Key_Down || key == Qt::Key_Up))
    return parentSplit ? parentSplit->navigatePane(key, tofind, parentSplit->indexOf(this)) : NULL;
  ...
  int nextind;
  if (ind == -1 && (ind = splitind) == -1) return NULL;
